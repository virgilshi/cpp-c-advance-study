...
int main()
{
    ...
    for (int i = 0; i <= ans; i++)
    {
        ...
        if (i > 0)
        {
            cout<<abs(q[i][0]-q[i-1][0])<<"Missionariers and "<<abs(q[i][1]-q[i-1][1])<<"Cannibals";
            if (q[i][2])
                cout<<"Take the boat from the right bank to the left bank"<<endl; 
            else
                cout<<"Take the boat from the left bank to the right bank"<<endl; 
//            cout<<"左岸有"<<q[i][0]<<"个传教士和"<<q[i][1]<<"个野人"<<endl; 
							cout<<"There are "<<q[i][0]<<" Missionariers and "<<q[i][1]<<" Cannibals in the left bank"<<endl; 
//            	cout<<"右岸有"<<n-q[i][0]<<"个传教士和"<<n-q[i][1]<<"个野人"<<endl<<endl; 
							cout<<"There are "<<n-q[i][0]<<" Missionariers and "<<n-q[i][1]<<" Cannibals in the right bank"<<endl;
        }
    }
